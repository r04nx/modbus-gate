...
const getDataSize = (dataType) => {
    switch (dataType) {
        case 'BOOL':
        case 'BOOLEAN': return 1; // Coils/Discrete Inputs take 1 address slot
        case 'INT16':
        ...
        case 'STRING': return 10; // Defaulting STRING to 10 registers (20 chars) to be safe/conservative
        default: return 1;
    }
};
...
export default function Servers() {
    ...
    // Pagination Logic
    const currentMappings = activeTab === 'MQTT_PUBLISHER' ? (config.config.publications || []) : (config.config.mappings || []);
    const totalPages = Math.ceil(currentMappings.length / itemsPerPage);
    const paginatedItems = currentMappings.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    const validationResult = useMemo(() => {
        ...
        const mappings = config.config.mappings || [];
        const publications = config.config.publications || [];
        ...
        if (activeTab === 'MODBUS_SERVER') {
                ...
                for (let i = start; i < end; i++) {
                    ...
                    if (occupied[key]) {
                        ...
                        errors.push({
                            ...
                            message: `Overlap: Slave ${slaveId} ${regType} Address ${i} used by multiple tags.`
                        });
                        conflictIndices.add(idx);
            ...
            mappings.forEach((m, idx) => {
                ...
                if (used[ioa]) {
                    errors.push({
                        ...
                        message: `Duplicate IOA: ${ioa} (mapped to multiple tags)`,
                        index: idx
                    });
                    conflictIndices.add(idx);
            ...
            mappings.forEach((m, idx) => {
                ...
                if (m.node_name) {
                    if (usedNames[m.node_name]) {
                        errors.push({
                            type: 'DUPLICATE_NODE',
                            node_name: m.node_name,
                            message: `Duplicate Node Name: '${m.node_name}' used multiple times.`,
                            index: idx
                        });
                        conflictIndices.add(idx);
                        conflictIndices.add(usedNames[m.node_name].index);
                    } else {
                        usedNames[m.node_name] = { index: idx, tag_id: m.tag_id };
                    }
                }
            });
        } else if (activeTab === 'MQTT_PUBLISHER') {
            const usedTopics = {}; // Key: broker_id:topic
            publications.forEach((p, idx) => {
                const key = `${p.broker_id}:${p.topic}`;
                if (usedTopics[key]) {
                    errors.push({
                        type: 'DUPLICATE_TOPIC',
                        topic: p.topic,
                        message: `Duplicate Publication: Topic '${p.topic}' for the same broker.`,
                        index: idx
                    });
                    conflictIndices.add(idx);
                    conflictIndices.add(usedTopics[key].index);
                } else {
                    usedTopics[key] = { index: idx, id: p.id };
                }
            });
        }

        return { errors, conflictIndices };
    }, [config.config.mappings, config.config.publications, activeTab, availableTags, config.config.slave_id]);
    ...
    const importFromCSV = (event) => {
        ...
        reader.onload = (e) => {
            ...
            const isModbusConflict = (m1, m2) => {
                // Must span different slaves or register types to be non-conflicting
                if (m1.slave_id !== m2.slave_id) return false;
                if (m1.register_type !== m2.register_type) return false;

                // If same slave/type, check address overlap
                const start1 = parseInt(m1.address);
                const end1 = start1 + getDataSize(m1.data_type);
                const start2 = parseInt(m2.address);
                const end2 = start2 + getDataSize(m2.data_type);

                return (start1 < end2 && start2 < end1);
            };

            const isModbusEqual = (m1, m2) => {
                return m1.tag_id === m2.tag_id &&
                    m1.slave_id === m2.slave_id &&
                    m1.register_type === m2.register_type &&
                    m1.address === m2.address &&
                    m1.data_type === m2.data_type;
            };

            const isOpcUaConflict = (m1, m2) => {
                // Node names should be unique
                return m1.node_name === m2.node_name;
            };

            const isOpcUaEqual = (m1, m2) => {
                return m1.tag_id === m2.tag_id && m1.node_name === m2.node_name;
            };

            const isIec104Conflict = (m1, m2) => {
                // Global IOA (base + offset) must be unique
                const ioa1 = parseInt(m1.base_value || 0) + parseInt(m1.ioa || 0);
                const ioa2 = parseInt(m2.base_value || 0) + parseInt(m2.ioa || 0);
                return ioa1 === ioa2;
            };

            const isIec104Equal = (m1, m2) => {
                const ioa1 = parseInt(m1.base_value || 0) + parseInt(m1.ioa || 0);
                const ioa2 = parseInt(m2.base_value || 0) + parseInt(m2.ioa || 0);
                return m1.tag_id === m2.tag_id && ioa1 === ioa2;
            };

            const isMqttConflict = (p1, p2) => {
                // Broker + Topic combination should be unique
                return p1.broker_id === p2.broker_id && p1.topic === p2.topic;
            };

            const isMqttEqual = (p1, p2) => {
                // Relaxed equality for MQTT: if everything matches, it's equal
                return p1.broker_id === p2.broker_id &&
                    p1.topic === p2.topic &&
                    p1.interval === p2.interval &&
                    p1.payload_template === p2.payload_template &&
                    JSON.stringify(p1.tags) === JSON.stringify(p2.tags);
            };
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (activeTab === 'MODBUS_SERVER') {
                    const tagId = getVal(row, 'tag_id');
                    if (!tagId) { validationError = "Missing Tag ID"; }
                    else if (!availableTags.some(t => t.tag_id === tagId)) { validationError = `Tag ID '${tagId}' not found`; }
                    else {
                        candidate = {
                            tag_id: tagId,
                        ...
                        // Check Self-Conflicts (within CSV)
                        const selfConflict = newMappings.find(m => isModbusConflict(m, candidate));
                        if (selfConflict) {
                            validationError = `CSV Duplication: Overlaps with row ${newMappings.indexOf(selfConflict) + 2} (Address ${selfConflict.address})`;
                        } else {
                            // Check Existing Conflicts
                            const existingConflict = existingMappings.find(m => isModbusConflict(m, candidate));
                            if (existingConflict) {
                                if (isModbusEqual(existingConflict, candidate)) {
                                    // Exact match - Skip silently (Idempotent)
                                    candidate = null;
                                } else {
                                    validationError = `Conflict with Existing: Slave ${candidate.slave_id}/${candidate.register_type} Address ${candidate.address} overlaps with mapped tag '${existingConflict.tag_id}'`;
                                }
                            }
                        }
                    }
                    ...
                    if (!tagId) { validationError = "Missing Tag ID"; }
                        ...
                        const selfConflict = newMappings.find(m => isOpcUaConflict(m, candidate));
                        if (selfConflict) validationError = `CSV Duplication: Node Name '${nodeName}' already in CSV`;
                        else {
                            const existingConflict = existingMappings.find(m => isOpcUaConflict(m, candidate));
                            if (existingConflict) {
                                if (isOpcUaEqual(existingConflict, candidate)) candidate = null;
                                else validationError = `Conflict with Existing: Node Name '${nodeName}' already mapped to tag '${existingConflict.tag_id}'`;
                            }
                        }
                    }
                } else if (activeTab === 'IEC104_SERVER') {
                    ...
                    if (!tagId) { validationError = "Missing Tag ID"; }
                        ...
                        const selfConflict = newMappings.find(m => isIec104Conflict(m, candidate));
                        if (selfConflict) validationError = `CSV Duplication: IOA ${candidate.base_value + candidate.ioa} already in CSV`;
                        else {
                            const existingConflict = existingMappings.find(m => isIec104Conflict(m, candidate));
                            if (existingConflict) {
                                if (isIec104Equal(existingConflict, candidate)) candidate = null;
                                else validationError = `Conflict with Existing: IOA ${candidate.base_value + candidate.ioa} already mapped to tag '${existingConflict.tag_id}'`;
                            }
                        }
                    }
                } else if (activeTab === 'MQTT_PUBLISHER') {
                    const brokerId = getVal(row, 'broker_id') || row[0];
                    if (!brokerId) validationError = "Missing Broker ID";
                    else {
                        ...
                        const selfConflict = newPublications.find(p => isMqttConflict(p, candidate));
                        if (selfConflict) validationError = `CSV Duplication: Topic '${candidate.topic}' for Broker '${candidate.broker_id}' already in CSV`;
                        else {
                            const existingConflict = existingPubs.find(p => isMqttConflict(p, candidate));
                            if (existingConflict) {
                                if (isMqttEqual(existingConflict, candidate)) candidate = null;
                                // For MQTT, maybe we allow same topic since it's just a publication? 
                                // But user requested "no duplicate entries". 
                                // Let's stick to unique topic per broker to be safe.
                                else validationError = `Conflict with Existing: Topic '${candidate.topic}' already exists for this broker.`;
                            }
                        }
                    }
                }
                ...
                if (validationError) {
                    ...
                    if (activeTab === 'MQTT_PUBLISHER') newPublications.push(candidate);
                    else newMappings.push(candidate);
                }
            }
        ...
        <div className="space-y-8 relative">
            ...
            <div className="bg-surface/50 backdrop-blur-md border border-surfaceHighlight rounded-2xl overflow-hidden shadow-card">
                ...
                <div className="p-8">
                    ...
                    {validationErrors.length > 0 && (
                        <div className="mb-8 bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-start gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div className="p-2 bg-red-500/20 rounded-lg">
                                <AlertTriangle size={24} className="text-red-500" />
                            </div>
                            <div className="flex-1">
                                <h4 className="text-lg font-bold text-red-500 mb-1">Configuration Errors Detected</h4>
                                <p className="text-text-secondary text-sm mb-3">Found {validationErrors.length} issues that must be resolved before saving.</p>
                                <div className="max-h-32 overflow-y-auto space-y-1 pr-2 scrollbar-thin scrollbar-thumb-surfaceHighlight scrollbar-track-transparent">
                                    {validationErrors.slice(0, 5).map((err, i) => (
                                        <div key={i} className="text-xs text-red-400 font-mono bg-red-500/5 px-2 py-1 rounded border border-red-500/10">
                                            {err.message}
                                        </div>
                                    ))}
                                    {validationErrors.length > 5 && (
                                        <div className="text-xs text-red-400 font-medium pl-1">
