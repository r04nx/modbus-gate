...
const JsonEditor = ({ value, onChange, availableTags = [] }) => {
    ...
    return (
        <div className="flex flex-col gap-2 border border-surfaceHighlight rounded-xl overflow-hidden bg-surface">
            {/* Toolbar */}
            <div className="flex items-center gap-2 p-2 bg-surfaceHighlight/10 border-b border-surfaceHighlight/30">
                <button
                ...
                <div className="relative">
                    ...
                    {showTagSelector && (
                        <div className="absolute top-full left-0 mt-1 w-80 max-h-80 bg-surface border border-surfaceHighlight rounded-xl shadow-xl z-10 flex flex-col">
                            <div className="p-2 border-b border-surfaceHighlight/30">
                                <input
                                    ...
                                    className="w-full bg-surfaceHighlight/10 border border-surfaceHighlight/30 rounded-lg px-2 py-1 text-xs text-white focus:outline-none focus:border-primary"
                                    autoFocus
                                />
                            </div>
                            <div className="flex-1 overflow-y-auto p-1">
                                {filteredTags.map(tag => (
                                    <div
                                        key={tag.tag_id}
                                        className="group flex flex-col gap-1 p-2 hover:bg-surfaceHighlight/10 rounded-lg border border-transparent hover:border-surfaceHighlight/20 transition-all"
                                    >
                                        <div className="flex items-center justify-between">
                                            <span className="font-medium text-xs text-text-primary truncate">{tag.name}</span>
                                            <span className="text-[10px] text-text-muted font-mono">{tag.tag_id}</span>
                                        </div>
                                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button
                                                onClick={() => insertTag(tag, 'value')}
                                                className="flex-1 flex items-center justify-center gap-1 bg-surfaceHighlight/20 hover:bg-primary/20 hover:text-primary text-[10px] py-1 rounded border border-surfaceHighlight/30 transition-colors"
                                                title="Insert Value Placeholder: {{tag_id}}"
                                            >
                                                <Hash size={10} />
                                                <span>Value</span>
                                            </button>
                                            ...
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-1 bg-surfaceHighlight/20 hover:bg-primary/20 hover:text-primary text-[10px] py-1 rounded border border-surfaceHighlight/30 transition-colors"
                                                title='Insert Key-Value: "Name": {{tag_id}}'
                                            >
                                                <KeyRound size={10} />
                                                <span>KV Pair</span>
                                            </button>
            ...
            {/* Footer */}
            <div className="px-4 py-2 bg-surfaceHighlight/5 border-t border-surfaceHighlight/30 text-xs text-text-muted flex justify-between">
