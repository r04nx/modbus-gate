...
sshpass -p "$REMOTE_PASS" ssh -o StrictHostKeyChecking=no "$REMOTE_USER@$REMOTE_HOST" "chmod +x $REMOTE_DIR/scripts/setup_remote.sh && cd $REMOTE_DIR && ./scripts/setup_remote.sh"
